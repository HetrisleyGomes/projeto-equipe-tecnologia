...
class RequisicoesRepository:
    ...
    def set_comments_requisition(self, id, comment) -> List:
        ...
        if result:
            ...
            if comentarios_json:
                ...
                try:
                    ...
                    comentarios = []  # Inicializar como lista vazia se o JSON estiver corrompido
            else:
                comentarios = []
        else:
            comentarios = []
        ...
        cursor.execute('UPDATE requisicoes SET comments = ? WHERE id = ?', (comentarios_json, id))
    ...
    def edit_requisition(self, id, propriety, status) -> None:
        ...
        cursor.execute('UPDATE requisicoes SET priority = ?, status = ? WHERE id = ?', (propriety, status, id,))
    ...
    def finalizar_requisition(self, id, data_atual) -> None:
        ...
        cursor.execute('UPDATE requisicoes SET data_conclusao = ?, priority = 0, status = "Finalizado" WHERE id = ?', (data_atual, id,))
